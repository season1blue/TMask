...
def annotate(sent):
    global client
    if client is None:
        client = CoreNLPClient(default_annotators='ssplit,tokenize'.split(','), url='http://localhost:9000')
    words = []
...
class Turn:
    ...
    def __init__(self, turn_id, transcript, turn_label, belief_state, system_acts, system_transcript, mask, history, num=None):
        self.id = turn_id
    ...
    def to_dict(self):
        return {'turn_id': self.id, 'transcript': self.transcript, 'turn_label': self.turn_label, 'belief_state': self.belief_state, 'system_acts': self.system_acts, 'system_transcript': self.system_transcript, 'mask':self.mask, 'history':self.history, 'num': self.num}
    ...
    @classmethod
    def annotate_raw(cls, raw, history=None):
        system_acts = []
    ...
    def numericalize_(self, vocab):
        ...
        # print(self.num['transcript'])
        # print(['<sos>'] + [w.lower() for w in self.transcript + ['<eos>']])
        # print(self.history)
        # print(self.transcript)
        # print("------------")
        num_history = []
        for h in self.history:
            num_h = vocab.word2index(['<sos>'] + [w.lower() for w in h + ['<eos>']], train=True)
            num_history.append(num_h)
        self.num['history'] = num_history
...
class Dialogue:
    ...
    def annotate_raw(cls, raw):
        ...
        for t in raw['dialogue']:
            ...
            history.append(annotate(t['system_transcript']))
            history.append(annotate(t['transcript']))
            curr_history = deepcopy(history)
            turn_ann, fail = Turn.annotate_raw(t, curr_history)
            # if fail:
            #     print(raw['dialogue_idx'])
